...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 27 22:01:55 2025
...
CLASSES = ["Unified"]
DATASET_PATHS = [
    "/home/mdb/DL_Lab3/ST_DATASETS/UAM/{}",
]

ROOT_BASE = "/home/mdb/reid-strong-baseline/Hyperparameter_tuning_SR"
...
reranking_options = ['no', 'yes']
base_lrs = [0.00035, 0.001]
center_loss_weights = [0.001]
range_margins = [0.3]
range_betas = [1.0]
weight_decays = [0.0001, 0.001]
max_epochs_list = [30, 60, 90]
...
for CLASS_NAME in CLASSES:
    ...
    total_experiments = len(CLASSES) * len(DATASET_PATHS) * len(combinations) * 2  # (el *2 es por "special" y "universal")
    experiment_counter = 0  # contador global

    for i, (model, loss, rerank, lr, clw, r_margin, r_beta, wd, max_ep) in enumerate(combinations):
        for size_variant in ["special", "universal"]:
            try:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                # Definir el tamaño de imagen en texto
                if size_variant == "special":
                ...
                # Ahora construimos el nombre bonito
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                exp_name = f"{size_text}_{dataset_tag}_EP{max_ep}_{timestamp}"

                exp_dir = os.path.join(output_root, exp_name)
                os.makedirs(exp_dir, exist_ok=True)
                total_experiments = len(CLASSES) * len(DATASET_PATHS) * len(combinations) * 2  # <-- MULTIPLICAS POR 2
                experiment_counter += 1
                print(f"\n[{i+1}-{size_variant}] Ejecutando experimento: {exp_name} ({i+1} de {len(combinations)} para {CLASS_NAME}, {experiment_counter} de {total_experiments} en total)")
                ...
                # Asignar tamaño de imagen según tipo de experimento
                if size_variant == "special":
                    if CLASS_NAME == "Crosswalks":
                        config['INPUT']['SIZE_TRAIN'] = [320, 96]
                        config['INPUT']['SIZE_TEST'] = [320, 96]
                    elif CLASS_NAME == "Containers":
                        config['INPUT']['SIZE_TRAIN'] = [128, 160]
                        config['INPUT']['SIZE_TEST'] = [128, 160]
                    elif CLASS_NAME == "Rubish":
                        config['INPUT']['SIZE_TRAIN'] = [160, 224]
                        config['INPUT']['SIZE_TEST'] = [160, 224]
                else:  # universal 256x128
                    config['INPUT']['SIZE_TRAIN'] = [256, 128]
                    config['INPUT']['SIZE_TEST'] = [256, 128]
